...
class DataCubit extends Cubit<DataState> {
  ...
  void filterData({
    ...
    filterList = allDataList.where((item) {
      ...
      return (item.city_name.toString().toLowerCase().contains(city) &&(
          item.district_name.toString().removeAllWhitespace.toLowerCase().contains(district.removeAllWhitespace) &&
          itemPrice <= maxPrice));
      // }
    }).toList();
  ...
  void filterProperties({ required String city,
    ...
    filterList = allDataList.where((item){
      final itemPrice = double.tryParse(item.price ?? "0") ?? 0;
      return (city.isEmpty || item.city_name!.toLowerCase().contains(city.toLowerCase()))
          && (district.isEmpty || item.district_name!.toLowerCase().contains(district.toLowerCase().trim()))
          && (itemPrice <= maxPrice);
    }).toList();
